...
OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate(const Channel requestedChannel) {
  // This fork publishes to GitHub Releases only. The inherited OTA proxy
  // resolves upstream firmware; never offer that image as a fork update.
  (void)requestedChannel;
  LOG_INF("OTA", "Fork OTA is unavailable; install the matching GitHub release from SD or USB");
