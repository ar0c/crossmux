...
bool isValidWebName(const String& name) {
  return FsHelpers::isValidPathComponent(std::string_view{name.c_str(), name.length()});
}
}  // namespace
    ...
    case WStype_TEXT: {
      ...
      if (msg.startsWith("START:")) {
        ...
        if (firstColon > 0 && secondColon > 0) {
          ...
          if (!isValidWebName(wsUploadFileName) || isProtectedItemName(wsUploadFileName)) {
