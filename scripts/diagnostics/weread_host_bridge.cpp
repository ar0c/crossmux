...
int main(int argc, char** argv) {
  try {
    ...
    if (action=="inspect" && argc==2) {
      std::cout<<"{\"account\":\""<<id.account<<"\",\"book\":\""<<id.book
        <<"\",\"source\":\""<<id.source<<"\",\"day\":"<<id.day
        <<",\"state\":"<<unsigned(bridge.ledger().state())<<",\"remaining\":"<<bridge.ledger().remaining()
        <<",\"verified\":"<<bridge.ledger().verified()<<",\"batch\":"<<bridge.ledger().batchSeconds()
    ...
    if(action=="audit" && argc==7) {
      snapshot.sampledAt=number(2); snapshot.month=number(3); snapshot.day=number(4);
      snapshot.monthSeconds=number(5); snapshot.daySeconds=number(6);
    ...
    if(action=="isolate" && argc==3) {
      ...
      snapshot.sampledAt=number(4); snapshot.month=number(5); snapshot.day=number(6);
      snapshot.monthSeconds=number(7); snapshot.daySeconds=number(8);
