...
struct Diagnostic {
  ...
  const char* name() const {
    switch (stage) {
      case Stage::None:
        return "none";
      case Stage::StartupMemory:
        return "startup_memory";
      case Stage::StartupSession:
        return "startup_session";
      case Stage::StartupAudit:
      ...
      case Stage::ReaderId:
        return "reader_id";
      case Stage::ReaderRequest:
        return "reader_request";
      case Stage::ReaderSignature:
        return "reader_signature";
      case Stage::ProgressMemory:
        return "progress_memory";
      case Stage::ProgressRequest:
        return "progress_request";
      case Stage::ProgressPayload:
        return "progress_payload";
      case Stage::TocOpen:
        return "toc_open";
      case Stage::TocRead:
        return "toc_read";
      case Stage::ChapterMissing:
        return "chapter_missing";
      case Stage::ProgressRange:
        return "progress_range";
      case Stage::ProgressFraction:
        return "progress_fraction";
      case Stage::SessionSave:
        return "session_save";
      case Stage::Clock:
        return "clock";
      case Stage::CloudKey:
        return "cloud_key";
      case Stage::CloudStats:
        return "cloud_stats";
      case Stage::Enter:
        return "enter";
      case Stage::Report:
  ...
  int encode(char* out, size_t capacity, unsigned queue, unsigned phase, unsigned issue, unsigned long tick) const {
    ...
    const int n = std::snprintf(
        ...
        "\"uptime_ms\":%lu,"
        "\"retries\":%u,\"network\":{\"stage\":%u,\"error\":%d,\"socket\":%d,\"tls\":%d,\"verify\":%d,\"elapsed_ms\":%"
        "u},"
        "\"report\":{\"attempted\":%u,\"accepted\":%u,\"seconds\":%u,\"prepare_age_ms\":%u,\"entry_gap_ms\":%u,"
        "\"has_previous\":%u,\"previous_gap_ms\":%u,\"elapsed_ms\":%u,\"response_bytes\":%u,\"http\":%d,\"error\":%d,"
        "\"guard\":%u,\"succ\":%d,\"synckey\":%d}}\n",
        name(), error, http, detail, queue, phase, issue, tick, unsigned(preparationRetries), unsigned(network.stage),
        network.error, network.socket, network.tls, network.verify, unsigned(network.elapsedMs),
