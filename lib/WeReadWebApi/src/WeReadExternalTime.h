...
struct ExternalTime {
  ...
  bool decode(const uint8_t* bytes, size_t size) {
    if (!bytes || size != kSize || std::memcmp(bytes,"WRTX",4) || bytes[4] != 1 ||
        number(bytes+232) != checksum(bytes,232)) return false;
    ...
    Ledger validator;
    if (!validator.bind(candidate.identity.account,candidate.identity.book,candidate.identity.source,candidate.identity.day)) return false;
    ...
    if (candidate.coveredSeconds > candidate.sourceMs/1000 || candidate.confirmedSeconds > candidate.coveredSeconds ||
        candidate.unknownSeconds != candidate.coveredSeconds-candidate.confirmedSeconds) return false;
    *this = candidate;
    return true;
  }
  bool balance(const Ledger& ledger, uint64_t& sendableSeconds) const {
    ...
    if (id.day != identity.day || std::strcmp(id.account,identity.account) || std::strcmp(id.book,identity.book) ||
        ...
        ledger.status() != Status::Pending || coveredSeconds > sourceMs/1000 ||
        confirmedSeconds > coveredSeconds || unknownSeconds != coveredSeconds-confirmedSeconds) return false;
