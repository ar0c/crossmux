...
class HandoverManifest {
 public:
  ...
  bool begin(File& file, const Identity& identity) {
    ...
    identity_ = identity;
    return match(
        file, std::snprintf(text_, sizeof(text_), "{\n  \"account\": \"%s\",\n  \"book\": \"%s\",\n  \"receipts\": [\n",
  ...
  bool finish(File& file, const Identity& id) {
    closed_ = previous_ && id.day == previous_ && same(id) &&
              match(file, std::snprintf(text_, sizeof(text_),
                                        "\n  ],\n  \"schema\": 1,\n  \"sender_enabled\": false,\n  \"source\": "
                                        "\"%s\",\n  \"state\": \"prepared\"\n}",
                                        id.source)) &&
              offset_ == file.fileSize64();
  ...
  bool deviceOwnedDay(const Identity& id) const { return closed_ && same(id) && id.day > previous_; }
