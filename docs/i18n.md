...
Startup-guide completion is stored in `onboardingVersion`. The guide is
skipped only when this value exactly matches `CURRENT_ONBOARDING_VERSION`;
missing, older, and newer values all run the guide. Increment the constant when
a firmware update must present a revised guide.
...
That legacy file is migrated once into `settings.json`.

### Content profile migration

Unified firmware persists `contentProfile` separately for rollback compatibility,
but keeps it synchronized with `language`: `ZH_CN` selects China and every other
language selects Global. Changing the UI language updates the service host but
preserves app visibility. Existing mismatched regions are corrected and resaved
on load. The selector lists all 33 generated languages.
