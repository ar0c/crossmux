...
struct Transport : TimeTransport {
  ...
  Read prepare(const Identity& id) override {
    ++prepares;
    assert(!std::strcmp(id.account, "a") && !std::strcmp(id.book, "b"));
    if (failedPrepares) {
      --failedPrepares;
      return Read::Failed;
    }
  ...
  Write enter() override {
    ++entries;
    return rejectEntry ? Write::Unknown : Write::Accepted;
  }
...
struct Fixture {
  ...
  Fixture() {
    std::strcpy(receipt.identity.account, "a");
    std::strcpy(receipt.identity.book, "b");
    std::strcpy(receipt.identity.source, "s");
    receipt.identity.day = 20709;
    receipt.sourceMs = 600000;
    transport.log = &log;
...
int main() {
  ...
  }
  {
    ...
    PacedJournal reboot(f.log);
    assert(reboot.open(f.receipt));
  ...
  for (const char* invalid :
       {"", "{}", "[]", "{\"succ\":1,}", "{\"succ\":1}garbage", "{\"succ\":1,\"succ\":1}", "{\"data\":{\"succ\":1}}",
        "{\"succ\":\"1\"}", "{\"succ\":1.0}", "{\"succ\":01}", "{\"succ\":1e0}", "{\"succ\":truex}",
  ...
  {
    Fixture f;
    f.transport.credit = 0;
    f.toSend();
    f.transaction.step();
    f.transport.advance(5);
    f.transaction.step();
    assert(f.transaction.step() == State::ReadbackWait);
    f.transport.credit = 30;
    f.transport.advance(5);
    f.transaction.step();
    assert(f.transaction.step() == State::Confirmed && f.transport.sends == 1);
  }
  ...
  {
    Fixture f;
    f.transport.failAfterSend = true;
    f.toSend();
  ...
  for (int phase = 0; phase < 6; ++phase) {
    Fixture f;
    for (int i = 0; i < phase; ++i) f.transaction.step();
    f.transaction.cancel();
  ...
  {
    Fixture f;
    f.transport.now = 1789401600 + 86400 - 10;
    f.toSend();
  ...
  }
  {
    ...
    f.toReserve();
    Log secondLog;
    PacedJournal second(secondLog);
    assert(second.open(f.receipt));
    TimeTransaction other(second, f.coordinator, f.transport);
    assert(other.begin());
    assert(other.step() == State::Waiting);
    f.transaction.cancel();
    assert(other.step() == State::Waiting);
    f.transport.advance(30);
    assert(other.step() == State::Preparing);
  }
