...
AccountSnapshot sample(uint64_t at, uint64_t total = 16234, uint64_t day = 2947) {
  ...
  result.month = 1788192000;
  result.day = 1789401600;
  result.monthSeconds = total;
  result.daySeconds = day;
  result.sampledAt = at;
...
int main(int argc, char** argv) {
  ...
  external.coveredSeconds = 3000;
  external.confirmedSeconds = 2880;
  external.unknownSeconds = 120;
  const uint64_t at = 1789453168;
  for (const bool torn : {false, true}) {
    MemoryLog log;
    PacedJournal journal(log);
    assert(journal.open(external));
  ...
  {
    PacedLedger bounds;
    assert(bounds.initialize(external));
  ...
  }
  {
    MemoryLog mixed;
    PacedLedger old;
    assert(old.initialize(external));
    uint8_t frame[PacedLedger::kSize];
    assert(old.encode(frame));
    ...
    PacedJournal bootstrap(mixed);
    assert(bootstrap.open(external));
    auto baseline = sample(at, 16234, 0);
    baseline.complete = false;
    baseline.monthComplete = true;
    assert(bootstrap.reserve(baseline, at, true));
    assert(mixed.bytes[PacedLedger::kSize + 4] == 4 && mixed.bytes[PacedLedger::kSize + 6] == 1);
    PacedJournal powerLoss(mixed);
    assert(powerLoss.open(external));
    ...
    PacedJournal resumed(mixed);
    assert(resumed.open(external));
    assert(!resumed.verify(sample(at + 2, 16264, 0)));
    auto absent = sample(at + 2, 16264, 30);
    absent.complete = false;
    absent.monthComplete = true;
    assert(!resumed.verify(absent));
    assert(resumed.verify(sample(at + 2, 16264, 30)));
    PacedJournal credited(mixed);
    assert(credited.open(external));
  ...
  changed.coveredSeconds = 0;
  changed.confirmedSeconds = 0;
  changed.unknownSeconds = 0;
  ...
  for (bool torn : {false, true}) {
    ...
    broken.torn = torn;
    broken.readError = !torn;
    assert(!writer.reserve(sample(at), at, true));
    assert(!writer.takeSendPermit());
    broken.readError = false;
    PacedJournal recovered(broken);
    if (torn)
      assert(!recovered.open(external));
    else
      assert(recovered.open(external) && !recovered.takeSendPermit());
  }
  PacedGate gate(UINT32_MAX - 10000);
  assert(!gate.ready(5000));
  assert(gate.ready(20000));
  gate.response(20000);
  assert(!gate.ready(10000));
  assert(!gate.ready(49999));
  assert(gate.ready(50000));
  std::cout << "Paced 30s journal: reboot, one-use permit, exact readback, unknown isolation, torn writes, monotonic "
               "gate PASS\n";
  for (int i = 1; i < argc; ++i) {
    if (!std::strcmp(argv[i], "--journal")) {
      ...
      MemoryLog imported;
      imported.bytes.resize(static_cast<size_t>(size));
      file.seekg(0);
      assert(file.read(reinterpret_cast<char*>(imported.bytes.data()), size));
      PacedLedger first;
      assert(first.decode(imported.bytes.data()));
      ExternalTime seed;
      seed.identity = first.identity();
      seed.sourceMs = first.measuredMs();
      seed.coveredSeconds = seed.unknownSeconds = ExternalTime::number(imported.bytes.data() + 184);
      PacedJournal replay(imported);
      assert(replay.open(seed));
      assert(!replay.takeSendPermit());
      std::cout << "Actual journal replay: frames=" << size / PacedLedger::kSize
                << " verified=" << replay.ledger().verified() << " pending=" << replay.ledger().batchSeconds()
                << " PASS\n";
      if (replay.ledger().state() != PacedLedger::State::Idle && replay.ledger().responseAt()) {
        ...
        PacedJournal restarted(imported);
        assert(restarted.open(seed));
