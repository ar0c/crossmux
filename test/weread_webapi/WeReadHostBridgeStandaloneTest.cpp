...
void continuousBatches() {
  ...
  ExternalTime source;
  source.identity = id.identity();
  source.sourceMs = 105 * 60 * 1000 + 17000;
  PacedLedger initial;
  ...
  baseline.month = 1788192000;
  baseline.day = 1789488000;
  baseline.sampledAt = 1789549000;
  baseline.monthSeconds = 10000;
  baseline.daySeconds = 100;
  ...
  for (unsigned batch = 0; batch < 106; ++batch) {
    ...
    observed.monthSeconds += seconds;
    observed.daySeconds += seconds;
...
int main() {
  ...
  source.identity = id.identity();
  source.sourceMs = 6325686;
  source.coveredSeconds = 3000;
  source.confirmedSeconds = 2880;
  source.unknownSeconds = 120;
  PacedLedger initial;
  assert(initial.initialize(source));
  ...
  baseline.day = 1789488000;
  baseline.sampledAt = 1789549000;
  baseline.monthSeconds = 16579;
  baseline.daySeconds = 178;
  baseline.complete = baseline.monthComplete = true;
  ...
  auto observed = baseline;
  observed.sampledAt += 10;
  observed.monthSeconds += 60;
  observed.daySeconds += 60;
  assert(reopened.settle(true, baseline.sampledAt + 2, observed));
  ...
  observed.daySeconds -= 53;
  observed.monthSeconds -= 53;
