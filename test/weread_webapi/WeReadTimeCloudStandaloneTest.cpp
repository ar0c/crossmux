...
bool parse(Response& response, const std::string& json, size_t chunk = 1) {
  for (size_t offset = 0; offset < json.size(); offset += chunk) {
    if (!response.feed(reinterpret_cast<const uint8_t*>(json.data() + offset), std::min(chunk, json.size() - offset)))
      return false;
  }
...
int main(int argc, char** argv) {
  ...
  for (size_t chunk : {1U, 7U, 4096U}) {
    ...
    assert(parse(
        response,
        R"({"baseTime":1788192000,"totalReadTime":16234,"readTimes":{"1789401600":2947},"ignored":[{"baseTime":0}]})",
        chunk));
    assert(response.snapshot().monthSeconds == 16234);
    assert(response.snapshot().daySeconds == 2947 && response.snapshot().hasDay);
  }
  for (const char* json : {R"({"baseTime":0,"totalReadTime":1,"readTimes":{}})",
  ...
  for (const char* json : {R"({"apikey":"wrk-a","apikey":"wrk-b"})", R"({"apikey":"wrk-evil\r\n"})",
                           R"({"apikey":null})", R"({"apikey":"wrk-a","errcode":1})"}) {
    response.reset(Response::Mode::Key);
    assert(!parse(response, json));
  }
  response.reset(Response::Mode::Key);
