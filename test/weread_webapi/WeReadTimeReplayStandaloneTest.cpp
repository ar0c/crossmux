...
Result requestVerified(const char* url, const RequestOptions& options, const DataCallback& data, const HeaderCallback&,
  ...
  for (size_t offset = 0; offset < bytes.size(); offset += fragment)
    if (!data(reinterpret_cast<const uint8_t*>(bytes.data() + offset), std::min(fragment, bytes.size() - offset)))
...
struct ReplayTransport : TimeTransport {
  ...
  Read snapshot(AccountSnapshot& out) override {
    if (!querying) {
      assert(query.begin("wr_vid=test; wr_skey=test", now));
      querying = true;
    }
    ...
    Identity identity{};
    std::strcpy(identity.account, "test");
...
void run(const std::string& json, uint64_t now, bool hasDay, int status = 200,
  ...
  for (size_t chunk : {1U, 7U, 257U, 4096U}) {
    ...
    std::strcpy(receipt.identity.account, "test");
    std::strcpy(receipt.identity.book, "book");
    std::strcpy(receipt.identity.source, "replay");
    receipt.identity.day = 20708;
    receipt.sourceMs = 105 * 60 * 1000;
    assert(journal.open(receipt));
    auto original = log.bytes;
    SendCoordinator coordinator(0);
    ReplayTransport transport(now);
    ...
    if (expected == WeReadTimeCloud::Result::Ready) {
      ...
      transport.now += 6;
      transport.tick += 6000;
      payload = "{\"baseTime\":" + std::to_string(baseline.month) +
                ",\"totalReadTime\":" + std::to_string(baseline.monthSeconds + 30) + ",\"readTimes\":{\"" +
      ...
      PacedJournal reopened(log);
      assert(reopened.open(receipt));
...
void bootstrapAmbiguity() {
  ...
  const std::string later =
      R"({"baseTime":1788192000,"totalReadTime":130,"readTimes":{"1789401600":100,"1789488000":30}})";
  ...
  std::strcpy(receipt.identity.account, "test");
  std::strcpy(receipt.identity.book, "book");
  std::strcpy(receipt.identity.source, "bootstrap-check");
  receipt.identity.day = 20708;
  receipt.sourceMs = 105 * 60 * 1000;
  PacedLedger ledger;
  assert(ledger.initialize(receipt));
  ...
  uint8_t encoded[PacedLedger::kSize];
  assert(ledger.encode(encoded));
  PacedLedger restarted;
  assert(restarted.decode(encoded));
  auto mismatch = observed;
  mismatch.monthSeconds += 30;
  assert(!restarted.verify(mismatch));
  mismatch = observed;
  mismatch.complete = false;
  assert(!restarted.verify(mismatch));
  mismatch = observed;
  mismatch.daySeconds = 60;
  assert(!restarted.verify(mismatch));
