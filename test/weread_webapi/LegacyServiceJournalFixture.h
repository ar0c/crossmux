// Frozen v1 reader/writer from dce3dbd; compatibility oracle, do not modernize.
#pragma once
#include <cstdio>
...
class LegacyServiceJournal {
 public:
  ...
  bool reserve(const char* device) {
    if (!ready_ || !device || !*device || std::strlen(device) >= sizeof(device_) || !matchesDevice(device) ||
        (state_ != State::Empty && state_ != State::Confirmed) || end_ >= measured_ || !log_.canReserve(3 * kSize))
      return false;
  ...
  bool jobId(char* out, size_t size) const {
    const int n = std::snprintf(out, size, "s-%s-%lu-%llu-%llu", id_.source, static_cast<unsigned long>(id_.day),
                                static_cast<unsigned long long>(start_), static_cast<unsigned long long>(end_));
 ...
 private:
  ...
  bool decodeNext() {
    ...
    if (std::memcmp(b, "WRS1", 4) || b[4] != 1 || b[5] < 1 || b[5] > 3 || b[6] || b[7] ||
        ExternalTime::number(b + 248) != ExternalTime::checksum(b, 248) || std::memcmp(b + 8, id_.account, 32) ||
        std::memcmp(b + 40, id_.book, 64) || std::memcmp(b + 104, id_.source, 64) ||
        ExternalTime::number(b + 168) != id_.day || ExternalTime::number(b + 208) != base_ ||
        !std::memchr(b + 176, 0, 32) || !b[176])
      return false;
    ...
    if (state_ == State::Empty) {
      ...
      } else if (start != start_ || end != end_ || unsigned(next) <= unsigned(state_))
        return false;
    }
    start_ = start;
    end_ = end;
  ...
  bool commit() {
    ...
    put(scratch_ + 216, start_);
    put(scratch_ + 224, end_);
