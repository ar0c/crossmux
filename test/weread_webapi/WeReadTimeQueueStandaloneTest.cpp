...
struct Source : TimeQueueSource, ByteLog {
  ...
  ExternalTime receipt(unsigned i) {
    ExternalTime r;
    std::strcpy(r.identity.account, "a"); std::strcpy(r.identity.book, "b"); std::strcpy(r.identity.source, "s");
    r.identity.day = 20708+i; r.sourceMs = measured[i];
    if (i == 1) { r.coveredSeconds = 3000; r.confirmedSeconds = 2880; r.unknownSeconds = 120; }
    return r;
  }
...
struct Transport : TimeTransport {
  ...
  Read snapshot(AccountSnapshot& out) override {
    ...
    out = {}; strcpy(out.account,"a"); out.month = 1788192000; out.day = 1789401600;
    out.monthSeconds = month; out.daySeconds = day; out.sampledAt = now; out.complete = true;
    return Read::Ready;
  }
  ...
  Write report(uint32_t seconds) override {
    ...
    lastSend = tick; ++sends; month += increment; day += increment;
  ...
  void advance() { ++now; tick += 1000; }
};
struct Fixture {
  Source source; Transport transport{source}; SendCoordinator coordinator{0};
  ...
  void until(T phase) {
    for (unsigned i=0;i<500 && queue.phase()!=phase;++i) { queue.step(); transport.advance(); }
...
int main() {
  ...
  for (unsigned fault=0; fault<5; ++fault) {
    ...
    switch(fault) {
      case 0: f.queue.cancel(); break;
      case 1: f.transport.failRead=true; break;
      case 2: f.transport.month-=60; f.transport.day-=60; break;
      case 3: f.transport.month-=30; f.transport.day-=30; break;
      case 4: f.source.logs[f.source.active].fail=true; break;
    }
    assert(f.finish(f.queue)==(fault==4 ? Q::StorageError : Q::Uncertain));
    f.transport.now+=180; f.transport.tick+=180000;
    f.source.logs[f.source.active].fail=false; f.transport.failRead=false;
  ...
  {
    Fixture f; f.queue.cancel(); f.source.full=true;
    TimeQueue bulk(f.source,f.source.journal,f.coordinator,f.transport,
                   TimeTransaction::BatchMode::Bounded60);
    ...
    TimeQueue reopen(f.source,f.source.journal,f.coordinator,f.transport,
                     TimeTransaction::BatchMode::Bounded60);
    assert(reopen.begin() && f.finish(reopen)==Q::Complete && f.transport.sends==sends);
  }
  {
    Fixture f; f.transport.unknown=true;
    assert(f.finish(f.queue)==Q::Uncertain && f.transport.sends==1);
    f.transport.now+=121; f.transport.tick+=121000;
    f.transport.unknown=false; f.source.full=true;
  ...
  }
  {
    ...
    for (int i=0;i<20;++i) { f.queue.step(); f.queue.cancel(); }
  ...
  {
    Fixture f; f.until(T::ReadbackWait); f.queue.cancel();
    ...
    for (int i=0;i<100 && recovery.confirmed()==0;++i) { recovery.step(); f.transport.advance(); }
    assert(recovery.confirmed()==30 && f.transport.sends==1 && f.transport.entries==1);
    recovery.cancel();
  }
  {
    Fixture f; f.until(T::Sending); f.queue.cancel(); // Reserved, never submitted.
  ...
  {
    Fixture f; f.until(T::ReadbackWait); f.queue.cancel(); f.transport.now+=121; f.transport.tick+=121000;
    TimeQueue recovery(f.source,f.source.journal,f.coordinator,f.transport);
    assert(recovery.begin() && f.finish(recovery)==Q::Uncertain && f.transport.sends==1);
  }
  {
    Fixture f; f.transport.credit=31;
  ...
  {
    Fixture f; f.source.error=true;
    assert(f.finish(f.queue)==Q::StorageError && f.transport.sends==0);
  }
  {
    Fixture f; f.until(T::Reserving); f.source.logs[f.source.active].fail=true;
    assert(f.finish(f.queue)==Q::StorageError && f.transport.sends==0);
  }
  {
    Fixture f; f.until(T::Preparing); f.queue.cancel();
    assert(f.queue.state()==Q::Paused && f.transport.sends==0);
    TimeQueue resume(f.source,f.source.journal,f.coordinator,f.transport);
    assert(resume.begin() && f.finish(resume)==Q::Complete && f.transport.sends==149);
  }
  {
    Fixture f; f.until(T::ReadbackWait); f.transport.failRead=true;
    assert(f.finish(f.queue)==Q::Uncertain && f.transport.sends==1);
  }
  std::cout << "Queue: bounded60 77 batches + legacy30 149 batches; tails, cooldown, repeat, cancellation, partial/zero credit, storage failure and frozen recovery PASS\n";
}
